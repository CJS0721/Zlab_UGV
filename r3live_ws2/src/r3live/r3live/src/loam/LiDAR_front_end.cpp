...
void horizon_handler( const livox_ros_driver::CustomMsg::ConstPtr &msg )
{
    ...
    // std::cout<<"lidar num: "<<' '<<plsize<<std::endl;
    pl_corn.reserve( plsize );
    pl_surf.reserve( plsize );
...
void pub_func( pcl::PointCloud< PointType > &pl, ros::Publisher pub, const ros::Time &ct )
{
    ...
    // std::cout<<"lidar time: "<<' '<<output.header.stamp<<std::endl;
    printf("lidar stamp : = %.3f,\n",output.header.stamp.toSec());
    pub.publish( output );
}
